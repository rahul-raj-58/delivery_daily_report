...
# ── Config ─────────────────────────────────────────────────────────────────────
CH_HOST     = os.getenv("CH_HOST",     "leio048s4j.us-east-1.aws.clickhouse.cloud")
CH_PORT     = int(os.getenv("CH_PORT", "8443"))
CH_USER     = os.getenv("CH_USER",     "rahul.raj")
CH_PASSWORD = os.getenv("CH_PASSWORD", "Jh1_IYZ%Czzl0nEi")

CACHE_TTL_SECONDS = int(os.getenv("CACHE_TTL", "300"))   # 5-minute cache

# ── App ────────────────────────────────────────────────────────────────────────
app = FastAPI(title="Spyne SLA Dashboard API", version="1.0.0")
...
# ── ClickHouse client (lazy singleton) ────────────────────────────────────────
_ch_client = None
...
async def sla_data(refresh: bool = Query(False, description="Force cache refresh")):
    """
    Returns day-wise and month-wise SLA / TAT metrics.
    Cached for 5 minutes (override with ?refresh=true).
    """
    cache_key = "sla_data"
    ...
    if not refresh:
        ...
        if cached is not None:
            logger.info("Cache hit for sla_data")
            return cached

    logger.info("Running SLA query against ClickHouse…")
    t0 = time.time()
    try:
    ...
    logger.info(f"Query returned {len(rows)} rows in {elapsed}s")
